...
package com.titankingdoms.dev.titanchat.tools.feed;
...
public final class UpdateFeedReader {
	...
	public UpdateFeedReader(String name, PluginDescriptionFile pdf) {
	...
	public String readFeed() {
		try {
			...
			XMLElement item = document.getElementsByName("item").get(0);
			
			if (item.isNode())
				throw new Exception();
			
			XMLElement title = item.getElementsByName("title").get(0);
			
			if (!title.isNode())
				throw new Exception();
			
			this.newName = title.getValue();
