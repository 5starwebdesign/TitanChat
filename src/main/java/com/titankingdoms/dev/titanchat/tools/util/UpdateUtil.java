...
package com.titankingdoms.dev.titanchat.tools.util;
...
import com.titankingdoms.dev.titanchat.tools.xml.*;
import com.titankingdoms.dev.titanchat.tools.xml.XMLObject.XMLType;
...
public final class UpdateUtil {
	...
	public String readFeed() {
		try {
			...
			if (!section.isType(XMLType.SECTION))
				throw new Exception();
			
			XMLObject element = ((XMLSection) section).getElements("title").get(0);
			
			if (!element.isType(XMLType.ELEMENT))
				throw new Exception();
