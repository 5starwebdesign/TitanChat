...
public class Loader {
	...
	public <T extends Loadable> List<T> load(Class<T> clazz, File directory) {
		...
		for (File file : directory.listFiles(new JarFilter())) {
			...
			try {
				...
				if (!result.getMessage().isEmpty())
					plugin.log(level, result.getMessage());
				
				if (!result.getResult())
					throw new Exception();
				
			} catch (Exception e) {
				plugin.log(Level.WARNING, "The Jar file " + file.getName() + " failed to load");
			}
